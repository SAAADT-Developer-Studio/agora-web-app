...
- **Light theme direction:** Slate desk + white editorial sheet. The page canvas should read as clearly colored cool slate (not near-white). Main content sits in a lifted white `content-sheet`. Use electric blue (`bias-right`) for active nav, primary CTAs, and category tags. Keep the black header. Dark mode removes the sheet and returns to flat surfaces.
